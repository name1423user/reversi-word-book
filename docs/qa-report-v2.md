...
> **状態：確認済みバグ2件はすべて修正済み**（2026-09-08）。
> 対象は `docs/qa-report.md`（既存バグ8件の修正）以降に入った変更 ——
> 「UIをQuizlet風に刷新」（複合状態遷移を持つ画面を広く書き換え）と
> 「AIに勉強資料からカードを作らせるプロンプトを追加」（JSON一括登録の
> 強化）。前回と同じ観点（「どうすれば壊せるか」）で、既存の確定済み
> 修正が壊れていないか・新しい変更が新しい壊れ方を持ち込んでいないかを
> 再攻撃した。
>
> 発見時は再現テストを `test.fail()` で記録し（前回と同じ運用）、
> 修正後に `e2e/regressions.spec.ts` へ回帰テストとして移した
> （`BUG-V2-001` / `BUG-V2-002` の2ブロック）。
...
- **分類**: Confirmed Bug
- **Severity**: Critical / 信頼度: High / 再現性: 100%（`e2e/regressions.spec.ts` の BUG-V2-001 ブロックで確定的に再現）
...
- **実際の実行による確認**: Yes（`e2e/regressions.spec.ts` の `BUG-V2-001` で、`RACE-CARD` が一括置き換え実行中にDBへ書き込まれた
...
- **修正済み**：削除対象のカードID集合を `buildCards`（時間のかかる画像
  処理）を実行する**前**に固定し、削除トランザクションはその固定IDリストに
  対して `bulkDelete` するようにした（`db.cards.where('deckId')...delete()`
  という「実行時点のデッキの中身」を対象にする問い合わせをやめた）。
  これにより、一括置き換えが処理中の間に別フォームで追加されたカードは、
  固定済みの削除対象リストに含まれないため巻き込まれない。
  （`src/components/Input/BulkPanel.tsx`）

### BUG-V2-002 ラウンド最後の1枚を判定した直後に「元に戻す」を押すと、カードの学習履歴は正しく巻き戻るのに、保存済みのセッション成績（次回比較の基準）だけ巻き戻らず永久にズレる

- **分類**: Confirmed Bug
- **Severity**: Medium / 信頼度: High / 再現性: 100%（`e2e/regressions.spec.ts` の BUG-V2-002 ブロックで確定的に再現。手動操作では極めて短い時間窓のためタイミング次第）
...
- **実際の実行による確認**: Yes（`e2e/regressions.spec.ts` の `BUG-V2-002` で、Undo後に `cards[0].history.length === 0` かつ
...
- **外部環境への依存**: なし（単一タブ内、IndexedDBの書き込み
  レイテンシに依存する時間窓）。
- **修正済み**：ラウンド最後の1枚を判定して `finishRound` を呼ぶ間、
  `isFinishing` フラグを立て、`finishRound` の永続化（`db.sessions.add`
  / `db.studyDays.put`）が完了するまでUndoボタンを無効化する（`disabled`
  属性、および`handleUndo`内部でも同フラグを見て早期returnする二重の
  ガード）ようにした。ロック中にDOM経由で直接クリックを発火させても
  （`disabled`なボタンはネイティブにclickイベントを発火しないため）
  何も起きない。ロックが外れる頃には`phase`が`'summary'`に切り替わって
  Undoボタン自体が画面から消えているため、ユーザー体験としては
  「ラウンド最後の1枚の判定直後は、ごく短い間Undoできないことがある」
  という程度の影響にとどまる。（`src/components/Flash/FlashPage.tsx`）
...
グリーンのままであることを確認した。UI刷新（Quizlet風リニューアル）は
見た目の書き換えが中心で、これらの修正が依拠していたデータフローの骨格
（`modify()`によるトランザクション化、参照カウント方式の画像削除、
下書きキーの一元化など）はそのまま残っている。
BUG-V2-001 / BUG-V2-002 の修正後は、`npx playwright test` の17件
（既存9件 + 本ラウンドで追加した2件 + `wordbook.spec.ts` の6件）が
すべて green。
